...
import type { Context, Pkg, Files } from '../context';

type RepoContext = {
  openStackRepo: string;
  currRepository: string;
  currBranch: string;
};
...
const dependencyLink = (name: string) => `https://www.npmjs.com/package/${name}`;

const getDependencyVersion = (dir: string, section: string, target: string) => {
...
const buildMardownPkgSection = (
  { analyze, tmp, prevVersion, nextVersion }: Context,
  { openStackRepo, currRepository, currBranch }: RepoContext
) => {
    ...
    Object.keys(pkg[section])
      ...
      .forEach((status) => {
        if (status === 'CONFLICT') {
          ...
          pkg[section][status].forEach((el: string) => {
...
const buildMardownFileSection = (
  { analyze, prevVersion, nextVersion }: Context,
  { openStackRepo, currRepository, currBranch }: RepoContext
) => {
  const files = analyze.files as Files;
  ...
  Object.keys(files).forEach((status) => {
    ...
    if (status === 'CONFLICT') {
      ...
      files[status].forEach((el: string) => {
...
const buildMarkdownReport = (context: Context) => {
...
const buildJsonReport = ({ inputs, analyze }: Context) => {
  fs.writeFileSync(inputs.report, JSON.stringify(analyze));
};

const getReportType = (file: string): string => (file.endsWith('.md') ? 'markdown' : 'json');
